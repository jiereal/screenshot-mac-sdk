  ...
  if ([self.snipView onKeyDown:event]) {
    [self.snipView
        setNeedsDisplayInRect:
            [self.window convertRectFromScreen:self.captureWindowRect]];
    return YES;
  }
  return NO;
}
...
- (void)doSnapshot:(NSScreen *)screen {
  ...
  SFLogInfo(@"SnipWindowController", @"screenShot finished, screen=%@", screen);
  NSRect mainFrame = [screen frame];
  self.originImage =
      [[NSImage alloc] initWithCGImage:imgRef size:mainFrame.size];
  self.darkImage = [[NSImage alloc] initWithCGImage:imgRef size:mainFrame.size];
  CGImageRelease(imgRef);
  SFLogInfo(@"SnipWindowController", @"CGImage released");

  // 对 darkImage 做暗化处理
  [self.darkImage lockFocus];
  [[NSColor colorWithCalibratedWhite:0 alpha:0.33] set];
  NSRectFillUsingOperation([SnipUtil rectToZero:mainFrame],
                           NSCompositeSourceAtop);
  [self.darkImage unlockFocus];
  SFLogInfo(@"SnipWindowController", @"darkImage dimmed");
}
...
- (void)captureAppScreen {
  ...
  for (NSDictionary *dir in [SnipManager sharedInstance].arrayRect) {
    ...
    CGRectMakeWithDictionaryRepresentation(
        (__bridge CFDictionaryRef)dir[(id)kCGWindowBounds], &windowRect);
  ...
  if ([SnipUtil isPoint:mouseLocation inRect:screenFrame]) {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:kNotifyMouseLocationChange
                      object:nil
                    userInfo:@{@"context" : self}];
  }
}

- (void)redrawView:(NSImage *)image {
  self.captureWindowRect =
      NSIntersectionRect(self.captureWindowRect, self.window.frame);
  if (image != nil &&
      (int)self.lastRect.origin.x == (int)self.captureWindowRect.origin.x &&
      ...
      (int)self.lastRect.size.height ==
          (int)self.captureWindowRect.size.height) {
    return;
  }
  if (self.snipView.image == nil && image == nil)
    return;
...
- (int)dragDirectionFromPoint:(NSPoint)point {
  if (NSWidth(self.captureWindowRect) <= kAdjustKnown * 2 ||
      NSHeight(self.captureWindowRect) <= kAdjustKnown * 2) {
    if (NSPointInRect(point, self.captureWindowRect)) {
      return 8;
    }
  }
  NSRect innerRect =
      NSInsetRect(self.captureWindowRect, kAdjustKnown, kAdjustKnown);
...
- (void)startCaptureWithScreen:(NSScreen *)screen {
  ...
  [self.window
      setBackgroundColor:[NSColor colorWithPatternImage:self.darkImage]];
  NSRect screenFrame = [screen frame];
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(onNotifyMouseChange:)
             name:kNotifyMouseLocationChange
           object:nil];
  [self showWindow:nil];
...
- (void)onNotifyMouseChange:(NSNotification *)notify {
  if (notify.userInfo[@"context"] == self)
    return;
  NSPoint mouseLocation = [NSEvent mouseLocation];
  if ([SnipManager sharedInstance].captureState == CAPTURE_STATE_HILIGHT &&
      self.window.isVisible &&
      [SnipUtil isPoint:mouseLocation inRect:self.window.screen.frame]) {
...
- (void)mouseDown:(NSEvent *)event {
  ...
  if ([SnipManager sharedInstance].captureState == CAPTURE_STATE_HILIGHT) {
    ...
    [self setupToolClick];
  } else if ([SnipManager sharedInstance].captureState ==
  ...
  if ([SnipManager sharedInstance].captureState == CAPTURE_STATE_EDIT &&
    ...
    if (NSPointInRect(mouseLocation, self.captureWindowRect)) {
      ...
      for (SFDrawPathInfo *info in self.snipView.pathView.rectArray) {
        NSPoint point =
            [self.window.contentView convertPoint:event.locationInWindow
                                           toView:self.snipView.pathView];
...
- (void)mouseUp:(NSEvent *)theEvent {
  if ([SnipManager sharedInstance].captureState ==
  ...
  if ([SnipManager sharedInstance].captureState != CAPTURE_STATE_EDIT) {
    ...
    if (self.rectDrawing) {
      ...
      if ([SnipManager sharedInstance].drawType == DRAW_TYPE_POINT ||
          [SnipManager sharedInstance].drawType == DRAW_TYPE_MOSAIC) {
        [self addPathInfo:[[SFDrawPathInfo alloc]
                              initWith:[self.linePoints copy]
                               andType:[SnipManager sharedInstance].drawType]];
      } else {
        if (currentInfo) {
          [self addPathInfo:currentInfo];
        } else {
          currentInfo = [[SFDrawPathInfo alloc]
                 initWith:self.rectBeginPoint
              andEndPoint:self.rectEndPoint
                  andType:[SnipManager sharedInstance].drawType];
          [self addPathInfo:currentInfo];
      ...
      if ([SnipManager sharedInstance].drawType != DRAW_TYPE_TEXT) {
        self.snipView.pathView.currentInfo.captureState =
...
- (void)mouseDragged:(NSEvent *)theEvent {
  if ([SnipManager sharedInstance].captureState ==
    ...
    self.captureWindowRect =
        NSUnionRect(NSMakeRect(self.startPoint.x, self.startPoint.y, 1, 1),
                    NSMakeRect(self.endPoint.x, self.endPoint.y, 1, 1));
    self.captureWindowRect =
        NSIntersectionRect(self.captureWindowRect, self.window.frame);
    ...
    case 8: {
      rect = NSOffsetRect(rect, self.endPoint.x - self.startPoint.x,
      ...
      if (!NSContainsRect(self.window.frame, rect)) {
        ...
        if (rect.origin.x >
            rcOrigin.origin.x + rcOrigin.size.width - rect.size.width) {
          rect.origin.x =
              rcOrigin.origin.x + rcOrigin.size.width - rect.size.width;
        }
        if (rect.origin.y >
            rcOrigin.origin.y + rcOrigin.size.height - rect.size.height) {
          rect.origin.y =
              rcOrigin.origin.y + rcOrigin.size.height - rect.size.height;
        }
        self.endPoint = NSMakePoint(
            self.startPoint.x + rect.origin.x - self.dragWindowRect.origin.x,
            self.startPoint.y + rect.origin.y - self.dragWindowRect.origin.y);
      }
    } break;
    case 7: {
...
- (void)endEditText {
  if (DRAW_TYPE_TEXT == [SnipManager sharedInstance].drawType) {
    ...
    if (currentInfo.drawHostView.superview != nil) {
      ...
      self.rectEndPoint = CGPointMake(
          self.rectBeginPoint.x + currentInfo.drawHostView.frame.size.width,
          self.rectBeginPoint.y - currentInfo.drawHostView.frame.size.height);
    }
    self.snipView.pathView.currentInfo = nil;
  }
}
...
- (void)addPathInfo:(SFDrawPathInfo *)pathInfo {
  ...
  [self.snipView
      setNeedsDisplayInRect:[self.window
                                convertRectFromScreen:self.captureWindowRect]];
...
- (NSImage *)capturedImageWithRect:(NSRect)rect {
  ...
  NSBitmapImageRep *bits =
      [[NSBitmapImageRep alloc] initWithFocusedViewRect:newRect];
  [self.originImage unlockFocus];

  if (bits.size.height == 0) {
    // initWithFocusedViewRect在10.14会为空，参考issues
    // https://github.com/MiMo42/MMTabBarView/pull/67 by 陈作华20190606
    [self.snipView removeDrawPath];
    NSBitmapImageRep *const imageRep =
        [self.snipView bitmapImageRepForCachingDisplayInRect:newRect];
  ...
  NSData *imageData =
      [bits representationUsingType:NSJPEGFileType properties:imageProps];
  NSImage *backgroundImage = [[NSImage alloc] initWithData:imageData];

  NSImage *pasteImage = nil;
  if (self.snipView.pathView.rectArray.count > 0) {
    self.snipView.pathView.layer.backgroundColor =
        [NSColor colorWithPatternImage:backgroundImage].CGColor;
...
- (void)onSave {
  __block NSImage *pasteImage =
      [self capturedImageWithRect:self.captureWindowRect];
  if (!pasteImage) {
    return;
  }
  NSLog(@"保存对话框jiereal");
  ...
  //  [[SnipManager sharedInstance] endCaptureWithImage:pasteImage needSave:YES
  ...
  [panel
      setNameFieldStringValue:[NSString stringWithFormat:@"%@.png", imageName]];
  ...
  [panel beginSheetModalForWindow:mainWindow
                completionHandler:^(NSModalResponse result) {
                  ...
                  [[SnipManager sharedInstance] endCaptureWithImage:pasteImage
                                                           needSave:YES
                                                   isHighResolution:YES];
                  if (result == NSModalResponseOK) {
                    ...
                    dispatch_async(dispatch_get_global_queue(0, 0), ^{
                      ...
                      //                    initWithCGImage:cgRef]; [newRep
                      //                    setSize:[pasteImage size]]; NSData
                      //                    *imageData = [newRep
                      //                    representationUsingType:NSPNGFileType
                      //                    properties:nil]; [imageData
                      //                    writeToURL:urlPath atomically:YES];
